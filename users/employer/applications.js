...
function getStatusBadge(status) {
  ...
  if (normalized === "interview") {
    return `<span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-700">Interview</span>`;
  }

  if (normalized === "rejected") {
    return `<span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-700">Rejected</span>`;
  }

  if (normalized === "hired") {
    return `<span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-700">Hired</span>`;
  }

  return `<span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-gray-100 text-gray-700">Applied</span>`;
}

function getActionButtons(app) {
  const normalized = String(app.status || "applied").toLowerCase();

  const interviewDisabled = normalized === "interview" ? "disabled" : "";
  const rejectedDisabled = normalized === "rejected" ? "disabled" : "";
  const hiredDisabled = normalized === "hired" ? "disabled" : "";

  return `
    <div class="flex gap-2 justify-end">
      <button ${interviewDisabled} onclick="event.stopPropagation(); updateApplicationStatus(${app.app_id}, 'interview', ${app.job_id}, '${(app.job_title || '').replace(/'/g, "\\'")}')" class="px-2.5 py-1 text-xs font-semibold rounded border border-blue-200 text-blue-700 bg-blue-50 hover:bg-blue-100 disabled:opacity-50 disabled:cursor-not-allowed">Interview</button>
      <button ${rejectedDisabled} onclick="event.stopPropagation(); updateApplicationStatus(${app.app_id}, 'rejected', ${app.job_id}, '${(app.job_title || '').replace(/'/g, "\\'")}')" class="px-2.5 py-1 text-xs font-semibold rounded border border-red-200 text-red-700 bg-red-50 hover:bg-red-100 disabled:opacity-50 disabled:cursor-not-allowed">Reject</button>
      <button ${hiredDisabled} onclick="event.stopPropagation(); updateApplicationStatus(${app.app_id}, 'hired', ${app.job_id}, '${(app.job_title || '').replace(/'/g, "\\'")}')" class="px-2.5 py-1 text-xs font-semibold rounded border border-green-200 text-green-700 bg-green-50 hover:bg-green-100 disabled:opacity-50 disabled:cursor-not-allowed">Hired</button>
    </div>
  `;
}
...
async function openApplicantsModal(jobId, jobTitle) {
  ...
  tbody.innerHTML =
    '<tr><td colspan="6" class="px-6 py-10 text-center text-gray-400"><i class="fas fa-circle-notch fa-spin mr-2"></i> Loading candidates...</td></tr>';
  ...
  try {
      ...
      result.data.forEach((app) => {
        ...
        const row = `
                    <tr onclick="openProfileModal(${app.user_id})" class="hover:bg-blue-50 transition group cursor-pointer border-b border-gray-50 last:border-0">
                        ...
                        <td class="px-6 py-4 align-top">${getStatusBadge(app.status)}</td>
    ...
    console.error(err);
    tbody.innerHTML =
      '<tr><td colspan="6" class="text-center text-red-500 py-6">Failed to load data.</td></tr>';
  }
}

async function updateApplicationStatus(appId, status, jobId, jobTitle) {
  try {
    const res = await fetch("../../api/jobs/update_application_status.php", {
      method: "POST",
    ...
    if (result.status) {
      await openApplicantsModal(jobId, jobTitle);
      return;
    }
    ...
    console.error(err);
    alert("Server communication failed while updating status.");
  }
}
...
async function openProfileModal(userId) {
  ...
  try {
    ...
    if (result.status) {
      ...
      document.getElementById("prof-bio").innerText = p.bio || "No bio provided.";
